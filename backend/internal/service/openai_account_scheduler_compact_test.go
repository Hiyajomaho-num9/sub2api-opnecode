...
func selectOpenAICompactionSchedulerTestAccount(t *testing.T, svc *OpenAIGatewayService, groupID int64, requireCompact bool) (*AccountSelectionResult, error) {
	...
	selection, _, err := svc.SelectAccountWithSchedulerForCapability(
		...
		nil,
		OpenAIUpstreamTransportAny,
		OpenAIEndpointCapabilityCompaction,
		requireCompact,
		false,
		false,
	)
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_NativeCompactionAllowsLocalChatBridge(t *testing.T) {
	for _, advanced := range []bool{false, true} {
		t.Run(map[bool]string{false: "legacy_scheduler", true: "advanced_scheduler"}[advanced], func(t *testing.T) {
			resetOpenAIAdvancedSchedulerSettingCacheForTest()
			svc := newOpenAICompactionSchedulerTestService([]Account{{
			...
			require.NoError(t, err)
			require.NotNil(t, selection)
			require.Equal(t, int64(71014), selection.Account.ID)
		})
	}
}
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_CompactionCapabilityAllowsLocalChatBridge(t *testing.T) {
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	groupID := int64(91008)
	account := Account{
		ID:          71080,
	...
	require.True(t, account.SupportsLocalOpenAICompactBridge())
	require.True(t, account.SupportsOpenAIEndpointCapability(OpenAIEndpointCapabilityCompaction))
	...
	require.True(t, isOpenAICompatibleAccountEligibleForRequestBeforeProfit(
		context.Background(), &account, PlatformOpenAI, "gpt-5.6-sol", false, OpenAIEndpointCapabilityCompaction,
	))
	svc := newOpenAICompactionSchedulerTestService([]Account{account}, false)

	selection, err := selectOpenAICompactionSchedulerTestAccount(t, svc, groupID, false)
	require.NoError(t, err)
	require.NotNil(t, selection)
...
func TestOpenAICompactSupportTier(t *testing.T) {
	tests := []struct {
		...
		{name: "chat fallback overrides native probe", account: &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Extra: map[string]any{"openai_compact_supported": false, openai_compat.ExtraKeyResponsesSupported: false}}, want: 2},
		...
		{name: "force off overrides local bridge", account: &Account{Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Extra: map[string]any{"openai_compact_mode": OpenAICompactModeForceOff, "openai_compact_supported": true}}, want: 0},
	}
