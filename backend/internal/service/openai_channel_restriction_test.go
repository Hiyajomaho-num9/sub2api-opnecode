...
func TestIsUpstreamModelRestrictedByChannel_PassthroughFlagWithRawChatFallbackMatchesForwardPath(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "native v2"},
		{name: "legacy compact", useCompactModelMapping: true, wantRestricted: true},
	} {
		tc := tt
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx := WithOpenAIForwardModel(
				context.Background(),
				"gpt-5.4-channel",
				tc.useCompactModelMapping,
			)

			require.Equal(t, tc.wantRestricted, svc.isUpstreamModelRestrictedByChannel(
				ctx, 10, account, "gpt-5.4", tc.useCompactModelMapping,
			))
		})
	}
}
